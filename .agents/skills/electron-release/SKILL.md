...
- **Pre-Build Verification**: Always run `yarn audit`, `yarn run typecheck`, `yarn run lint`, and `yarn run test` before builds.
...
1. Check dependencies for vulnerabilities: `yarn audit`
2. Verify code quality: `yarn run typecheck && yarn run lint`
3. Run test suite: `yarn run test && yarn run test:e2e`
4. Check bundle size: `yarn run build:analyzer`
...
| Large app download              | Analyze with `ANALYZE=true yarn run build`, enable compression |
