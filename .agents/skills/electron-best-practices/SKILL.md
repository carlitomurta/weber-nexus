...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  onUpdateCounter: (callback: (value: number) => void) => {
    const handler = (_event: IpcRendererEvent, value: number) =>
      callback(value);
...
```bash
tsx scripts/analyze-security.ts <path> [options]
...
Examples:
  # Analyze a project
  tsx scripts/analyze-security.ts ./src

  # Strict mode for CI pipeline
  tsx scripts/analyze-security.ts ./src --strict --json
```
...
```bash
tsx scripts/scaffold-electron-app.ts [options]
...
Examples:
  ...
  tsx scripts/scaffold-electron-app.ts \
...
```bash
tsx scripts/generate-ipc-types.ts [options]
...
Examples:
  ...
  tsx scripts/generate-ipc-types.ts \
